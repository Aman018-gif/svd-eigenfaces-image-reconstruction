...
The average image represents the "common structure" of all faces in the dataset.
2. Perturbation Matrix Construction
Each individual image is flattened into an N \times 1 column vector. The perturbation matrix (P) is constructed by subtracting the mean face vector  from each individual face vector.
